#!/usr/bin/env python3
"""Validate all strains identified only to genus level with Species equal to sp."""
...
def database_result() -> list[str]:
    return unique_sorted_accessions(
        ...
        lambda record: field_equals(record, "Species", "sp."),
    )


def main() -> None:
    accessions = database_result()
    write_accessions_tsv("Q027_species_sp.tsv", accessions)
