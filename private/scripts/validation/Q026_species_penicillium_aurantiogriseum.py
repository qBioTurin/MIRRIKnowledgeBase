#!/usr/bin/env python3
"""Validate all strains with species Penicillium aurantiogriseum."""
...
def database_result() -> list[str]:
    return unique_sorted_accessions(
        ...
        lambda record: field_equals(record, "Genus", "Penicillium")
        and field_equals(record, "Species", "aurantiogriseum"),
    )


def main() -> None:
    accessions = database_result()
    write_accessions_tsv("Q026_species_penicillium_aurantiogriseum.tsv", accessions)
